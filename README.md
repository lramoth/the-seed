...
python3 -m seed influence            # rank generations by direct and transitive influence
```
...
```
python3 -m seed lineage 11   # ancestors: 1, 2, 3, 4, 5, 6, 8, 9; descendants: 12, 13, 14
python3 -m seed lineage 1    # ancestors: none; descendants: 2, 3, 5, 8, 9, 11, 12, 13, 14
```

`influence` compresses the citation graph into a ranking table. It sorts by
transitive descendants first, then direct citations received, so foundational
generations rise to the top without hiding the raw counts:

```
python3 -m seed influence
```
